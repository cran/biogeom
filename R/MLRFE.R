MLRFE <- function(P, x, simpver = 1){
  ...
  if(xopt >= (xmin+xmax)/2){
      temp <- yopt*((z-xmax)*(z-xmin)^2/((xopt-xmin)*((xopt-xmin)*(z-xopt)
      -(xopt-xmax)*(xopt+xmin-2*z))))^delta
  }

  if(xopt < (xmin+xmax)/2){
      temp <- yopt*((z-xmin)*(z-xmax)^2/((xmax-xopt)*((xmax-xopt)*(z-xopt)
      -(xmin-xopt)*(xopt+xmax-2*z))))^delta
  }
