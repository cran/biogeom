...
fitPE <- function(x, y, ini.val, simpver = NULL, 
    ...
    obj.fun <- function(z){
        ...
        r.obs   <- sqrt( x.obs^2 + y.obs^2 )
        ########################################################################################
        #### To convert the observed phi values to the zeta values in the Preston equation
        a <- z[4] 
        if(a !=0 ){
          Value   <- r.obs/a*sin(phi.obs)
          indA    <- which(Value > 1)
          Value[indA] <- 1
          indB    <- which(Value < -1)
          Value[indB] <- -1
          indC    <- which(is.na(Value))
          if(length(indC) > 0) Value[indC] <- NA          
          zeta.obs <- asin(Value)
          if(TRUE){
            ind1    <- which(x.obs >= 0 & y.obs >=0)
            zeta.obs[ind1] <- zeta.obs[ind1]
            ind2    <- which(x.obs < 0 & y.obs >=0)
            zeta.obs[ind2] <- pi - zeta.obs[ind2]
            ind3    <- which(x.obs < 0 & y.obs < 0)
            zeta.obs[ind3] <- pi - zeta.obs[ind3]
            ind4    <- which(x.obs > 0 & y.obs < 0)
            zeta.obs[ind4] <- zeta.obs[ind4] + 2*pi    
          }
          #######################################################################################
          ResJ    <- PE(P=z[4:p], zeta=zeta.obs, simpver=simpver) 
          r.theo  <- ResJ$r
          TwoTab  <- cbind(r.obs, r.theo)
          TwoTab  <- na.omit(TwoTab)
          r.obs   <- TwoTab[,1]
          r.theo  <- TwoTab[,2]          
          RSS     <- sum( (r.obs - r.theo)^2 )
        }
        if(a==0) RSS <- Inf
        return( RSS )       
    }  
    ...
    ########################################################################################
    #### To convert the observed phi values to the theta values in the Preston equation
    a       <- par[4]
    ...
    ResJ    <- PE(P=par[4:p], zeta=zeta.obs, simpver=simpver) 
    r.theo  <- ResJ$r
    x.theo  <- ResJ$x
    y.theo  <- ResJ$y
    x2      <- x.theo
    ...
    CombTab <- cbind(x1, y1, x2, y2, zetaA)
    ...
    if(!is.null(simpver)){
      if( simpver != 3 & par[6] < 0 ){
        par[6]  <- -par[6]
        par[3]  <- (par[3] + pi) %% (2*pi)
        x1.temp <- x1*cos(pi) - y1*sin(pi)
        y1.temp <- y1*cos(pi) + x1*sin(pi) 
        x2.temp <- x2*cos(pi) - y2*sin(pi)
        y2.temp <- y2*cos(pi) + x2*sin(pi) 
        x1      <- x1.temp
        y1      <- y1.temp
        x2      <- x2.temp
        y2      <- y2.temp
    ...
    zeta.arr     <- seq(0, 2*pi, len=np)
    if( is.null(angle) ){        
        results  <- curvePE(P=par, zeta=zeta.obs, simpver=simpver)
        results2 <- curvePE(P=par, zeta=zeta.arr, simpver=simpver)
    ...
    if( !is.null(angle) ){        
        ...
        par.new[3] <- angle 
        ###########################################################################
        results  <- curvePE(P=par.new, zeta=zeta.obs, simpver=simpver)
        results2 <- curvePE(P=par.new, zeta=zeta.arr, simpver=simpver)
    ...
    x.range  <- range(x1)
    y.range  <- range(y1)
    Width    <- x.range[2] - x.range[1]
    Length   <- y.range[2] - y.range[1]    
    r1       <- sqrt(x1^2 + y1^2)
    r2       <- sqrt(x2^2 + y2^2)
    RSS      <- sum((r1 - r2)^2)
    ...
    para.tab <- data.frame( Parameter = c(Names, 
                    ...
                    Estimate = c(par, Length, Width, r.sq, RSS, length(x1)) )
    ...
    return(list( par=par, scan.length=Length, scan.width=Width, 
      scan.area=Area,  
      r.sq=r.sq, RSS=RSS, sample.size=length(x1),
      zeta.stand.obs=zetaA, zeta.trans=zetaB, 
